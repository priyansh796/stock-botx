...
# --- YOUR ORIGINAL FUNCTIONS (STRICTLY UNTOUCHED) ---
...
# --- NEW QUANT STRATEGY FUNCTIONS (SEPARATE LAYER) ---
...
for stock in stocks:
    ...
    try:
        ...
        if len(w_df) >= 300:
            ...
            rsi_w_ma = rsi_w.rolling(14).mean()

            # Original Weekly Entry Logic
            if (rolling_setup_weekly(w_df, 20) and rolling_cross(w_close, w_df['SSF_50'].values, 6) and 
                rsi_w.iloc[-1] > rsi_w_ma.iloc[-1] and w_df['SSF_50'].iloc[-1] < w_df['SSF_200'].iloc[-1]):
        ...
        if len(m_df) >= 80:
            ...
            rsi_m_ma = rsi_m.rolling(14).mean()

            # Original Monthly Entry Logic
            if (rolling_setup_monthly(m_df, 12) and rolling_cross(m_close, m_df['SSF_50'].values, 3) and 
                rsi_m.iloc[-1] > rsi_m_ma.iloc[-1] and m_df['SSF_50'].iloc[-1] < m_df['SSF_200'].iloc[-1]):
...
with pd.ExcelWriter(PORTFOLIO_FILE, engine="openpyxl", mode="w") as writer:
    ...
    # Appending Quant findings separately
