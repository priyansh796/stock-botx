...
def get_predictive_signal(stock_symbol, local_df):
    try:
        ...
        # FIX: Try multiple keys for basis, or use local SSF_20 as fallback
        bb_basis = ind.get("BB.basis") or ind.get("SMA20") or ind.get("Bollinger.B")
        if bb_basis is None:
            bb_basis = local_df['SSF_20'].iloc[-1]
        
        if all(v is not None for v in [ao, bb_upper, bb_lower, bb_basis]):
            bandwidth = (bb_upper - bb_lower) / bb_basis
            
            # Adjusted threshold to 0.18 based on market observation
            if bandwidth < 0.18: 
                if ao > 0:
                    return "PREDICT_UP", (1/bandwidth) * ao
                elif ao < 0:
                    return "PREDICT_DOWN", (1/bandwidth) * abs(ao)
                    
        return "HOLD", 0
    except Exception as e:
        print(f"TV Error for {stock_symbol}: {e}")
        return "HOLD", 0
