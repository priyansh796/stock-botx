...
from ta.volume import ChaikinMoneyFlowIndicator # Added for Institutional Activity
import os
import gspread
...
# --- NEW WEIGHTED PREDICTIVE ENGINE ---
def get_predictive_signal(stock_symbol, local_df):
    try:
        # 1. Institutional Activity (CMF)
        ...
        if all(v is not None for v in [ao, bb_u, bb_l, bb_m]):
            ...
            # GATE: Require at least 2 points for a signal
            if up_score >= 2:
...
msg2 = f"""🔮 SMART MONEY PREDICTIONS (Ranked)

Predictive UP (Top 15):
{predictive_up[:15]}

Predictive DOWN (Top 15):
{predictive_down[:15]}"""
